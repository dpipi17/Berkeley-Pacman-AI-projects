...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # temporary counter for update
        allPossible = util.Counter()
       
        # check if ghost is captured or not by Pacman
        if noisyDistance != None:
            for legalPos in self.legalPositions:
                trueDistance = util.manhattanDistance(legalPos, pacmanPosition)

                # formula: P(x(t), e(1:t)) proportional P(x(t)|e(1:t-1)) * P(e(t)|x(t))
                allPossible[legalPos] = self.beliefs[legalPos] * emissionModel[trueDistance]
        else:
            # situation  When a ghost is captured by Pacman, all beliefs should be updated
            # so that the ghost appears in its prison cell.
            allPossible[self.getJailPosition()] = 1.0

        
        # because we use proportional operator, we have to normalize probabilities
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # this codes is works same as formula:
        # P(x(t) | e(1:t-1)) = Sum(x(t-1)) (P(x(t-1) | e(1:t-1)) * P(x(t) | x(t-1)))
        
        # for every possible old Position
        for oldPos in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                # because of for loops we rewrite formula.
                # P(newPos(t) | e(1:t-1)) += P(oldPos(t-1) | e(1:t-1)) * P(newPos | oldPos)
                allPossible[newPos] += self.beliefs[oldPos] * prob 

        # there is = operator and we dont need to normalize
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # create variable which value is same as number of particles
        counter = self.numParticles

        # Particles should be evenly distributed across legal positions
        # thats why in loop we create new particles wich value is current legal position
        # repeat this process to create numParticles number particles 
        while counter > 0:
            for position in self.legalPositions:
                if counter == 0:
                    break
                counter -= 1
                # add a current legal position as a new particle
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            # temporary counter for update
            allPossible = util.Counter()
            # old beliefs before evidence
            beliefs = self.getBeliefDistribution()
            
            for particle in self.particleList:
                trueDistance = util.manhattanDistance(particle, pacmanPosition)

                # formula: B(x) proportional B'(x) * P(e|x)
                allPossible[particle] = beliefs[particle] * emissionModel[trueDistance]
            
            # we have to normalize our new beliefs, because proportional operator
            allPossible.normalize()

            # check if all weights of particles is zero or not
            if any(allPossible.values()):
                # if all particels dont recieve 0 weight
                # we have to update particles with new allPossible belief
                self.particleList = []
                for _ in range(0, self.numParticles):
                    self.particleList.append(util.sample(allPossible))
            else:
                # Situation When all particles receive 0 weight, they should be recreated from the prior distribution
                self.initializeUniformly(gameState)
        else:
            # situation When a ghost is captured by Pacman
            # so we have to update our particle list
            # all particle should be same (self.getJailPosition())
            self.particleList = []
            for _ in range(0, self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        # code bellow will update every particle using formula x' = sample(P(X'|x))


        # create temporary particle list to update particles positions
        tempParticleList = []

        # for every paticle old position
        for particleOldPos in self.particleList:

            # get the distribution over new positions for the ghost, given its previous position
            # this is P(X'|x) where x is particleOldPosition
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particleOldPos))

            # add new particle to temporary list
            # where new particle x' is sample(newPosDist). 
            # newPos = sample(P(X'|oldPos)). X' is new position possible values
    ...
    def getBeliefDistribution(self):
        ...
        # create counter to store agent's current belief
        beliefDistribution = util.Counter()

        # add 1 to every cell value where is particle 
        for particle in self.particleList:
            beliefDistribution[particle] += 1

        # normilize belief because we have to divide every cell value by particles number
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # create list where we store particles
        self.particleList = []

        # generate list where will be all possible ghosts possitions at the same time (all possible particles)
        # every element in this list will be a tuple of ghost positions
        allPossibleGhostPositions = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(allPossibleGhostPositions)
        ...
        while counter > 0:
            for positions in allPossibleGhostPositions:
                if counter == 0:
                    break
                counter -= 1
                # add a current legal position as a new particle
                self.particleList.append(positions)
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particleList:
            ...
            # create variable to store probability for this particle after evidence
            temp = 1
            
            for index in range(0, self.numGhosts):
                
                # get EmsissionModel for this ghost
                currEmissionModel = emissionModels[index]
                
                # check if ghost is captured
                if noisyDistances[index] == None:
                    # When a ghost is captured by Pacman, particle should be updated
                    # so that the ghost appears in its prison cell
                    particle = self.getParticleWithGhostInJail(particle, index)
                else:
                    trueDistance = util.manhattanDistance(particle[index], pacmanPosition)
                    
                    # update temporary probability variable for this particle after evidence (for current ghost)
                    temp *= currEmissionModel[trueDistance]
            
            # update probability of this particle after evidence
        ...
        if any(allPossible.values()):
            ...
            # any ghosts that are eaten (have noisyDistance of None) must be changed to the jail Position
            for particle in self.particleList:
                for index in range(0, self.numGhosts):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particleList:
            ...
            for index in range(0, self.numGhosts):
                # get the distribution over new positions for the current ghost, given its previous position
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), index, self.ghostAgents[index])
                
                # get and update position for current ghost
                # where new position is sample(newPosDist)
