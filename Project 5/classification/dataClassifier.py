...
def enhancedFeatureExtractorDigit(datum):
    ...
    # in this method i add three new features.
    # 1) if digit has one white cluster (in white i mean off pixel lights. (x, y) pixel is white if features[(x, y)] == 0)
    # 2) if digit has two white clusters
    # 3) if digit has three white clusters
    # these features improve algorithm work, because if digit has one cluster, its more likely to be 1, 2, 3, 5, 7,
    # if it has two clusters its more likely to by 6, 0, 9. if it has three clusters --> 8
    # 4 depends on who writes it.
    # i use dfs algorithm to find number of white clusters.
 
    features =  basicFeatureExtractorDigit(datum)
    # set for check if i already been in this pixel
    been = set()

    # counter to store number of white clusters
    numberOfWhiteClusters = 0

    # dfs algorithm method
    def dfs(currNode):
        # check if pixel coordinates are out of range, we should return method
        if currNode[0] < 0 or currNode[0] >= DIGIT_DATUM_HEIGHT or currNode[1] < 0 or currNode[1] >= DIGIT_DATUM_WIDTH:
            return
        
        # check if we already been in this pixel or pixel color is not white
        if currNode in been or features[(currNode[0], currNode[1])] == 1:
            return

        # mark this pixel as visited
        been.add(currNode)
        
        # call dfs function for every neighbour
        dfs((currNode[0] + 1, currNode[1]))
        dfs((currNode[0] + 1, currNode[1] + 1))
        dfs((currNode[0] + 1, currNode[1] - 1))
        dfs((currNode[0] - 1, currNode[1]))
        dfs((currNode[0] - 1, currNode[1] + 1))
        dfs((currNode[0] - 1, currNode[1] - 1))
        dfs((currNode[0], currNode[1] + 1))
        dfs((currNode[0], currNode[1] - 1))

    # two for loops to get every pixel 
    for row in range(0, DIGIT_DATUM_HEIGHT):
        for col in range(0, DIGIT_DATUM_WIDTH):
            nextNode = (row, col)

            # if we have not been in this pixel and its color is white
            # we should increase number of white clusters and call dfs from this pixel
            if nextNode not in been and features[(row, col)] == 0:
                numberOfWhiteClusters += 1
                dfs(nextNode)

    # add this tree features to features counter
    features['oneWhiteCluster'] = numberOfWhiteClusters == 1 
    features['twoWhiteCluster'] = numberOfWhiteClusters == 2
    features['threeWhiteCluster'] = numberOfWhiteClusters == 3
...
def enhancedPacmanFeatures(state, action):
    ...
    # in this method i add six new features.
    # 1) if state after this action is winning state
    # 2) if state after this action is loosing state
    # 3) score for the state after this state
    # 4) 1 / minimum distance to nearest food from pacman
    # 5) 1 / sum of ghostDistances from pacman position 
    # 6) 1 / ghostDanger parameter
    ...
    # get a state after the action from the starting state
    currentGameState = state.generateSuccessor(0, action)
    ...
    # add this six features to features counter
    features['isWin'] = currentGameState.isWin()
    features['isLoose'] = currentGameState.isLose()
    features['score'] = currentGameState.getScore()
    features['minDistanceToFood'] = 1 / float(minDistanceToFood)
    features['ghostDistance'] = 1 / float(ghostDist)
    features['ghostDanger'] = 1 / float(ghostDanger)
